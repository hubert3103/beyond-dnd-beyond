...
const BottomNavigation = ({ activeTab, onTabChange }: BottomNavigationProps) => {
  const tabs = [
    ...
    { id: 'items' as TabType, label: 'Items', icon: '/itemsIcon.svg' },
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-1 flex flex-col items-center py-3 px-2 ${
              activeTab === tab.id ? 'text-red-500' : 'text-gray-200'
            }`}
          >
            <img 
              src={tab.icon} 
              ...
              className={`w-6 h-6 mb-1 ${
                activeTab === tab.id ? 'filter brightness-0 saturate-100' : 'filter brightness-0 saturate-100'
              }`}
              style={{
